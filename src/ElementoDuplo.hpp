...
template <typename T> class ElementoDuplo {
private:
...
public:
   ...
   */
  ElementoDuplo<T> *getProximo() const { return _next; }
   ...
   */
  void setProximo(ElementoDuplo<T> *next) { _next = next; }
   ...
   */
  ElementoDuplo<T> *getAnterior() const { return this->_previous; }

  //! Método que modifica o ponteiro "anterior" do elemento.
  /*!
   * \param novo o ponteiro a ser modificado.
   * \sa getProximo(), getInfo(), setProximo(), getAnterior()
   */
  void setAnterior(ElementoDuplo<T> *novo) { _previous = novo; }
};
