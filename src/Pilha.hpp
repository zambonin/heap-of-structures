...
template <typename T> class Pilha : public EstruturaLinear<T> {
public:
   ...
   */
  void empilha(T dado) { this->adicionaDado(dado); }
   ...
   */
  T desempilha() { return this->retiraDado(); }
   ...
   * \sa topo()
   */
  int getPosTopo() { return this->getPosFinal(); }

  //! Método que limpa a pilha de quaisquer elementos.
  /*!
   * \sa empilha(), desempilha()
   */
  void limparPilha() { this->limparEstruturaLinear(); }
   ...
   * \sa pilhaCheia()
   */
  bool PilhaVazia() { return this->estruturaLinearCheia(); }
   ...
   * \sa pilhaVazia()
   */
  bool PilhaCheia() { return this->estruturaLinearVazia(); }
};
