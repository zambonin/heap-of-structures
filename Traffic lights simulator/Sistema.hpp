...
class Sistema {
 public:
    ...
    /*! Descreve o número de carros que passaram por qualquer semáforo do sistema. */
    int carrosQuePassaram;

    //! Contador de carros que entraram no sistema.
    /*! Descreve o número de carros total que foram criador por qualquer pista. */
    ...
    *   de execução total do programa. */
    ...
    //! Método que cria todas as pistas e semáforos de acordo com os dados fornecidos.
    /*!
    * \sa iniciar()
    */
    void instanciar();
    ...
    /*!
    * \sa gerarEventosCarro(), gerarEventosChegadaSemaforo(), gerarEventosAberturaSemaforos()
    */
    void iniciar();
    ...
    */
    ...
    //! Método que escreve na tela informações consideradas necessárias para o entendimento da saída do programa.
    /*!
    * \sa contarCarros()
    */
...
void Sistema::instanciar() {
    ...
    Semaforo* so1leste = new Semaforo(true, vetorPista1, probPista1, tempoSemaforo);  

    Pista* vetorPista2[4] = {c1leste, l1leste, n2norte, s2sul};
    int probPista2[3] = {40, 30, 30};
    Semaforo* sc1leste = new Semaforo(true, vetorPista2, probPista2, tempoSemaforo);

    Pista* vetorPista3[4] = {c1oeste, o1oeste, s1sul, n1norte};
    int probPista3[3] = {40, 30, 30};
    Semaforo* sc1oeste = new Semaforo(false, vetorPista3, probPista3, tempoSemaforo);

    Pista* vetorPista4[4] = {s1norte, n1norte, o1oeste, c1leste};
    int probPista4[3] = {10, 10, 80};
    Semaforo* ss1norte = new Semaforo(false, vetorPista4, probPista4, tempoSemaforo);
  
    Pista* vetorPista5[4] = {s2norte, n2norte, c1oeste, l1leste};
    int probPista5[3] = {30, 30, 40};
    Semaforo* ss2norte = new Semaforo(false, vetorPista5, probPista5, tempoSemaforo);
   
    Pista* vetorPista6[4] = {n1sul, s1sul, c1leste, o1oeste};
    int probPista6[3] = {10, 80, 10};
    Semaforo* sn1sul = new Semaforo(false, vetorPista6, probPista6, tempoSemaforo);

    Pista* vetorPista7[4] = {n2sul, s2sul, l1oeste, c1oeste};
    int probPista7[3] = {30, 40, 30};
    Semaforo* sn2sul = new Semaforo(false, vetorPista7, probPista7, tempoSemaforo);

    Pista* vetorPista8[4] = {l1oeste, c1oeste, s2sul, n2norte};
    int probPista8[3] = {30, 30, 40};
    Semaforo* sl1oeste = new Semaforo(false, vetorPista8, probPista8, tempoSemaforo);
...
void Sistema::executarEventos() {
    for (int i = 0; i < listaEventos->getTamanho(); i++) {
        ...
        switch (eventoAtual->getTipo()) {
            ...
            default: {
                std::cout << "Nunca deve chegar aqui se tudo estiver correndo certo." << std::endl;
            }
        }
...
void Sistema::finalizarPrograma() {
    ...
    std::cout << "Ratio: " << ((float) carrosQuePassaram * 100) / ((float) tempoDeExecucao) << " carros por segundo" << std::endl;
}
